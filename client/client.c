...
//Untuk akses data base harus punya username dan password
int auth(char *username, char *password){
	...
	if(found==0){
		printf("%s %s\n", username, password);	
		printf("Pastikan Password dan Username Benar\n");
		return 0;
	}else{
		return 1;
	}
...
int main(int argc, char *argv[]){
	...
	while(1){
        ...
        else if (strcmp(perintah[0], "CREATE")==0 && strcmp(perintah[1], "DATABASE")==0) {
        snprintf(buffer, sizeof buffer, "cDatabase:%s:%s:%d", perintah[2], argv[2], id_user);
				// printf("%s\n", buffer);
				send(clientSocket, buffer, strlen(buffer), 0);
        }       

		else if(strcmp(perintah[0], "CREATE")==0 && strcmp(perintah[1], "TABLE")==0){
		...
		else if(strcmp(perintah[0], "DROP")==0 && strcmp(perintah[1], "DATABASE")==0){
				snprintf(buffer, sizeof buffer, "dDatabase:%s:%s", perintah[2], argv[2]);
				send(clientSocket, buffer, strlen(buffer), 0);
		}
        
        else if(strcmp(perintah[0], "DROP")==0 && strcmp(perintah[1], "TABLE")==0){
				snprintf(buffer, sizeof buffer, "dTable:%s:%s", perintah[2], argv[2]);
				send(clientSocket, buffer, strlen(buffer), 0);
		}
        
        else if(strcmp(perintah[0], "DROP")==0 && strcmp(perintah[1], "COLUMN")==0){
				// printf("masuk\n");
